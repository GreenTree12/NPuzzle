...
public class Database1 {
    ...
    public static int getCost(String name) {
        ...
        String tableName = "fifteen_puzzle_2";
    ...
    public static void insertPatterns() {
        String goalState = "5,6,7,9,10,11,13,14,15";
        int d = 4;
        int n[][] = new int[4][4];
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
        ...
        list.add(goalState);
        Database1.insertCost(goalState, 0);
        int count = 0;
    ...
    public static void generatePatterns(String currentState) {
        ...
        String temp[] = null;
        int zerPos = Integer.parseInt(parts[8]);
        ...
        if (zeroRow > 0) {
           ...
           /* Database.arrayCopy(fullState, demo);
            ...
            String newState = String.format("%s,%s,%s,%s,%s,%s,%s,%s,%s", temp[0], temp[1], temp[2], temp[3], temp[4], temp[5], temp[6], temp[7], temp[8]);

            int cost = Database1.getCost(newState);
            if (cost == -1) {
                //System.out.printf("U,%s,cost=%d\n", newState, Database.getCost(currentState) + 1);
                list.add(newState);
                //printState(fullState);
                Database1.insertCost(newState,Database1.getCost(currentState)+1);
            }
        }
        if (zeroRow < d - 1) {
